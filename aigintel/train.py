...
@TinyJit
def test_step(model: BaseModel, x: Tensor, y: Tensor) -> Tensor:
    # TODO: Implement before showing the others
    pass

def shuffle_batch(x_train: Tensor, y_train: Tensor) -> (Tensor, Tensor):
    indices = Tensor(np.random.permutation(x_train.shape[0]))  # Randomly shuffle indices
...
def train(model: BaseModel, config: dict, args: Namespace):
    ...
    # x, y = Tensor.rand(4, 1, config["width"], config["height"]), Tensor([2,4,3,7])
    x_train, y_train, _, _ = mnist(fashion=config["fashion"])
    ...
    best_epoch: int = 0
    es = EarlyStopping(config["early_stopping"]["patience"], config["early_stopping"]["min_delta"])
    ...
    with Tensor.train():
        for i in (t:=trange(config["epochs"])):
            ...
            t.set_description(f"Epoch {i+1} | Avg Loss = {avg_loss:.6f}")
    ...
    plot_loss(losses)
    plot_loss(epoch_durations, "Duration", "Epoch")